...
export default function Login() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const res = await api.post<SigninResponse>('/signin', { username, password });
      ...
      <main className="flex-1 flex items-center justify-center px-4">
        <div className="w-full max-w-sm">
          ...
          <div className="border border-white/20 border-b-0 px-6 pt-6 pb-4">
            ...
            <h2 className="font-mono text-xl font-bold">LOGIN</h2>
          </div>
          ...
          <div className="border border-white/20 border-t-0 px-6 py-3">
            <p className="font-mono text-xs text-white/30">
              No account?{' '}
              <Link to="/signup" className="text-white/60 hover:text-white transition-colors underline">
                Sign up
              </Link>
            </p>
          </div>
